...
public class KCImageRequest extends KCHttpRequest<Bitmap>
{
	...
	/**
	 * Creates a new image request, decoding to a maximum specified width and height. If both width and height are zero, the image will be decoded to
	 * its natural size. If one of the two is nonzero, that dimension will be clamped and the other one will be set to preserve the image's aspect
	 * ratio. If both width and height are nonzero, the image will be decoded to be fit in the rectangle of dimensions width x height while keeping
	 * its aspect ratio.
	 *
	 * @param url
	 *            URL of the image
    	...
    	this.setResponseParser(new KCHttpResponseParser()
		{
			...
			public KCHttpResult<Bitmap> parseHttpResponse(KCHttpResponse response)
			{
				...
				synchronized (sDecodeLock)
				{
					...
					{
						KCLog.e("Caught OOM for %d byte image, url=%s", response.getContent().length, getUrl());
	 ...
	 */
	private KCHttpResult<Bitmap> doParse(KCHttpResponse response)
	{
		...
		if (bitmap == null)
		{
			return KCHttpResult.error(new KCParseError(response));
		}
		else
		{
			return KCHttpResult.success(bitmap, KCHttpHeaderParser.parseCacheHeaders(response));
		}
	}
