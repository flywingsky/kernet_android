...
import com.kercer.kernet.http.base.KCEncodingUtils;


public class KCMultipartUtils
{
	...
	public static final byte[] CRLF_BYTES = KCEncodingUtils.getAsciiBytes(CRLF);

	public static int getContentLengthForMultipartRequest(String boundary, Map<String, KCMultiPartRequest.KCMultiPartParam> multipartParams, Map<String, String> filesToUpload)
	{
		final int boundaryLength = boundary.getBytes().length;
		int contentLength = 0;
		for (String key : multipartParams.keySet())
		{
			KCMultiPartRequest.KCMultiPartParam param = multipartParams.get(key);
			int size = boundaryLength + CRLF_LENGTH + HEADER_CONTENT_DISPOSITION_LENGTH + COLON_SPACE_LENGTH + String.format(FORM_DATA, key).getBytes().length + CRLF_LENGTH
					+ HEADER_CONTENT_TYPE_LENGTH + COLON_SPACE_LENGTH + param.contentType.getBytes().length + CRLF_LENGTH + CRLF_LENGTH + param.value.getBytes().length
					+ CRLF_LENGTH;
		...
		for (String key : filesToUpload.keySet())
		{
			...
			int size = boundaryLength + CRLF_LENGTH + HEADER_CONTENT_DISPOSITION_LENGTH + COLON_SPACE_LENGTH
					+ String.format(FORM_DATA + SEMICOLON_SPACE + FILENAME, key, file.getName()).getBytes().length + CRLF_LENGTH + HEADER_CONTENT_TYPE_LENGTH + COLON_SPACE_LENGTH
					+ CONTENT_TYPE_OCTET_STREAM_LENGTH + CRLF_LENGTH + HEADER_CONTENT_TRANSFER_ENCODING_LENGTH + COLON_SPACE_LENGTH + BINARY_LENGTH + CRLF_LENGTH + CRLF_LENGTH;
